...
人类用户只和 SA 交互。其它 Agent 的问题、缺口、失败、超时和风险必须先回流到 SA，不得直接向用户提问或要求用户推动流程。
...
2. 读取 `workflow/handoff-contracts.md` 和 `workflow/traceability-chain.md`，确认 DESIGN 阶段必须把 AC 交接为设计落点、任务、运行契约、测试计划和允许写入范围。
3. 读取当前 `proposal.md`、`specs/**/spec.md`、`acceptance.md`、`docs/api/api.md`、`docs/database/database.md`、`docs/runtime/runtime-contract.md` 和 `docs/testing/testing.md`。
4. 在 `docs/runtime/runtime-contract.md` 填齐 frontend/backend/API/proxy/health、`delivery_e2e_command`、`browser_e2e_command`、`browser_e2e_user_actions`、`api_contract_doc`、`database_contract_doc`、`persistence_contract`、`mock_policy`。
5. 在 `docs/api/api.md` 维护 API / 数据 / Mock / Runtime 关系；在 `docs/database/database.md` 维护持久化方式和 API/Mock/Runtime 关系。没有数据库时必须写 `Not Required: <原因>`，不能留空。
6. 在当前 `test-plan.md` 要求 QA 计划 CI/CD、Delivery E2E / Runtime Smoke 和 Browser Interaction E2E；存在前端或管理端 AC 时，Browser E2E 不得省略。
7. 设计交付前运行 `python tools/check-gate-readiness.py --gate design --change <change-name> --change-id <CR-ID>`；失败时补设计、任务、测试计划或文档同步，不得要求 PL 直接通过。
...
- AC -> Design -> Task -> allowed_write_scope -> Test Plan 的追踪链不断；下游 FE/BE/Admin/AI/QA/Ops 不需要猜端口、API、数据来源、用户动作或 mock policy。
